...
// Session interface using UserRole from schema
interface Session {
 id: string;
 userId: number;
 username: string;
...
// Role-based access control using schema permissions
function hasPermission(session: Session, permission: string): boolean {
 ...
 return session.role === 'admin' || userPermissions.includes(permission);
}
...
app.post("/api/auth/register", async (req: Request, res: Response) => {
 ...
 if (!['admin', 'health_worker', 'doctor'].includes(role)) {
...
app.patch("/api/admin/users/:id/role", requireAuth('admin'), async (req: Request, res: Response) => {
 const { id } = req.params;
 const { role } = req.body;
 if (!role || !['admin', 'health_worker', 'doctor'].includes(role)) {
 return res.status(400).json({ error: 'Invalid role' });
 }
 try {
